...
<footer class="mdl-mini-footer">
      <div class="mdl-mini-footer__left-section">
        ...
        if ($this->menu == MENU_MAIN) {
            $this->addHtml(getIndexHtmlModal());
        }
...
function getIndexHtmlModal() {
    $html = new UiEditor(false);
    $html->addHtml('<div id="myModal" class="modal">
    ...
    $html->buildButton("button_add_manual", "Add")->setAdditionalClasses("button-mobile-fullsize")->setIsColoured()->setIsAccent()->setRaised()->setSubmit()->generate();
    $html->addCollapsable("Set Mode", getIndexHtmlCollapsibleButtons(), true, "button-mobile-fullsize");
    $html->addHtml('<br>
...
function getIndexHtmlCollapsibleButtons() {
    $bbconfig      = BBConfig::getInstance();
    $html          = new UiEditor(false);
    $html->addLineBreak();

        $html->buildButton("BARCODE_P", "Purchase")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_P"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_C", "Consume")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_C"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_CA", "Consume all")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_CA"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_CS", "Consume (spoiled)")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_CS"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_O", "Open")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_O"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_GS", "Inventory")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_GS"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_AS", "Add to shoppinglist")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_AS"] . "')")
            ->setRaised()
            ->generate();

        $html->buildButton("BARCODE_Q", "Set quantity")
            ->setOnClick("addBarcodeToTextbox('" . $bbconfig["BARCODE_Q"] . "')")
            ->setRaised()
            ->generate();
